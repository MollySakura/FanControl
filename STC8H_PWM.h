...
//						              7    6    5    4     3     2     1      0     Reset Value
//sfr PWMA_BKR = 0xFEDDH;  MOE1 AOE1 BKP1 BKE1 OSSR1 OSSI1 LOCK11 LOCK10  0000,0000  /* 刹车寄存器 */ 
//sfr PWMB_BKR = 0xFEFDH;  MOE2 AOE2 BKP2 BKE2 OSSR2 OSSI2 LOCK21 LOCK20  0000,0000  /* 刹车寄存器 */ 
...
#define PWMA_BrakeAutoOutputEnable()		PWMA_BKR |= 0x40		//1：自动输出使能
#define PWMA_BrakeAutoOutputDisable()		PWMA_BKR &= ~0x40		//0：自动输出禁止
#define PWMB_BrakeAutoOutputEnable()		PWMB_BKR |= 0x40		//1：自动输出使能
#define PWMB_BrakeAutoOutputDisable()		PWMB_BKR &= ~0x40		//0：自动输出禁止

#define PWMA_BrakeHighValid()						PWMA_BKR |= 0x20		//1：刹车输入高电平有效
#define PWMA_BrakeLowValid()						PWMA_BKR &= ~0x20		//0：刹车输入低电平有效
#define PWMB_BrakeHighValid()						PWMB_BKR |= 0x20		//1：刹车输入高电平有效
#define PWMB_BrakeLowValid()						PWMB_BKR &= ~0x20		//0：刹车输入低电平有效

#define PWMA_BrakeEnable(n)							(n==1?(PWMA_BKR |= 0x10):(PWMA_BKR &= ~0x10))		//1：开启刹车输入
#define PWMB_BrakeEnable(n)							(n==1?(PWMB_BKR |= 0x10):(PWMB_BKR &= ~0x10))		//1：开启刹车输入
//#define PWMA_BrakeEnable()							PWMA_BKR |= 0x10		//1：开启刹车输入
#define PWMA_BrakeDisable()							PWMA_BKR &= ~0x10		//0：禁止刹车输入
//#define PWMB_BrakeEnable()							PWMB_BKR |= 0x10		//1：开启刹车输入
#define PWMB_BrakeDisable()							PWMB_BKR &= ~0x10		//0：禁止刹车输入

//运行模式下“关闭状态”选择
#define PWMA_OSSRnEnable()							PWMA_BKR |= 0x08		//1：当 PWM 不工作时，一旦 CCiE=1 或 CCiNE=1，首先开启 OC/OCN 并输出无效电平，然后置OC/OCN 使能输出信号=1
#define PWMA_OSSRnDisable()							PWMA_BKR &= ~0x08		//0：当 PWM 不工作时，禁止 OC/OCN 输出（OC/OCN 使能输出信号=0）
#define PWMB_OSSRnEnable()							PWMB_BKR |= 0x08		//1：当 PWM 不工作时，一旦 CCiE=1 或 CCiNE=1，首先开启 OC/OCN 并输出无效电平，然后置OC/OCN 使能输出信号=1
#define PWMB_OSSRnDisable()							PWMB_BKR &= ~0x08		//0：当 PWM 不工作时，禁止 OC/OCN 输出（OC/OCN 使能输出信号=0）
//空闲模式下“关闭状态”选择
#define PWMA_OSSInEnable()							PWMA_BKR |= 0x04		//1：当 PWM 不工作时，一旦 CCiE=1 或 CCiNE=1，OC/OCN 首先输出其空闲电平，然后 OC/OCN使能输出信号=1
#define PWMA_OSSInDisable()							PWMA_BKR &= ~0x04		//0：当 PWM 不工作时，禁止 OC/OCN 输出（OC/OCN 使能输出信号=0）
#define PWMB_OSSInEnable()							PWMB_BKR |= 0x04		//1：当 PWM 不工作时，一旦 CCiE=1 或 CCiNE=1，OC/OCN 首先输出其空闲电平，然后 OC/OCN使能输出信号=1
#define PWMB_OSSInDisable()							PWMB_BKR &= ~0x04		//0：当 PWM 不工作时，禁止 OC/OCN 输出（OC/OCN 使能输出信号=0）
...
#define PWMA_LockLevelSet(n)						PWMA_BKR = (PWMA_BKR & ~0x03) | (n&3)		//锁定设置。该位为防止软件错误而提供的写保护措施
#define PWMB_LockLevelSet(n)						PWMB_BKR = (PWMB_BKR & ~0x03) | (n&3)		//锁定设置。该位为防止软件错误而提供的写保护措施
